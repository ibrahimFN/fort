...
async def event_party_invite(invitation):
    ...
    if data['loglevel'] == 'normal':
        ...
        if invitation.sender.display_name is None:
            print(f'[{now_()}] [{client.user.display_name}] None / {invitation.sender.id} からパーティー {invitation.party.id} への招待')
        else:
            print(f'[{now_()}] [{client.user.display_name}] {invitation.sender.display_name} / {invitation.sender.id} からパーティー {invitation.party.id} への招待')
            ...
            if client.acceptinvite is True:
                if client.acceptinvite_interval is True:
                    ...
                    try:
                        ...
                        if data['loglevel'] == 'normal':
                            ...
                            if invitation.sender.display_name is None:
                                print(f"[{now_()}] [{client.user.display_name}] None / {invitation.sender.id} からパーティー {invitation.party.id} への招待を{str(data['fortnite']['interval'])}秒拒否")
                            else:
                                print(f"[{now_()}] [{client.user.display_name}] {invitation.sender.display_name} / {invitation.sender.id} からパーティー {invitation.party.id} への招待を{str(data['fortnite']['interval'])}秒拒否")
                    except fortnitepy.PartyError:
                        if data['loglevel'] == 'debug':
                            print(red(traceback.format_exc()))
                ...
                try:
                    ...
                    if data['loglevel'] == 'normal':
                        ...
                        if invitation.sender.display_name is None:
                            print(f'[{now_()}] [{client.user.display_name}] None / {invitation.sender.id} からパーティー {invitation.party.id} への招待を拒否')
                        else:
                            print(f'[{now_()}] [{client.user.display_name}] {invitation.sender.display_name} / {invitation.sender.id} からパーティー {invitation.party.id} への招待を拒否')
                except fortnitepy.PartyError:
                    if data['loglevel'] == 'debug':
                        print(red(traceback.format_exc()))
                    print(red(f'[{now_()}] [{client.user.display_name}] 受信したnet_clとクライアントのnet_clが一致しません。'))
...
async def event_friend_add(friend):
    ...
    if friend.direction == 'INBOUND':
        if data['loglevel'] == 'normal':
            ...
            if friend.display_name is None:
                print(f'[{now_()}] [{client.user.display_name}] None / {friend.id} がフレンドに追加')
            else:
                print(f'[{now_()}] [{client.user.display_name}] {friend.display_name} / {friend.id} がフレンドに追加')
    else:
        if data['loglevel'] == 'normal':
            if friend.display_name is None:
            ...
            if friend.display_name is None:
                print(f'[{now_()}] [{client.user.display_name}] None / {friend.id} をフレンドに追加')
            else:
                print(f'[{now_()}] [{client.user.display_name}] {friend.display_name} / {friend.id} をフレンドに追加')
...
async def event_friend_remove(friend):
    ...
    if data['loglevel'] == 'normal':
        ...
        if friend.display_name is None:
            print(f'[{now_()}] [{client.user.display_name}] None / {friend.id} がフレンドから削除')
        else:
            print(f'[{now_()}] [{client.user.display_name}] {friend.display_name} / {friend.id} がフレンドから削除')
...
async def event_party_member_join(member):
    ...
    if data['loglevel'] == 'normal':
        ...
        if member.display_name is None:
            print(magenta(f'[{now_()}] [パーティー/{member.party.id}] [{client.user.display_name}] None / {member.id} [{member.platform}/{member.input}] がパーティーに参加\n人数: {member.party.member_count}'))
        else:
            print(magenta(f'[{now_()}] [パーティー/{member.party.id}] [{client.user.display_name}] {member.display_name} / {member.id} [{member.platform}/{member.input}] がパーティーに参加\n人数: {member.party.member_count}'))
...
async def event_party_member_leave(member):
    ...
    if data['loglevel'] == 'normal':
        ...
        if member.display_name is None:
            print(magenta(f'[{now_()}] [パーティー/{member.party.id}] [{client.user.display_name}] None / {member.id} [{member.platform}/{member.input}] がパーティーを離脱\n人数: {member.party.member_count}'))
        else:
            print(magenta(f'[{now_()}] [パーティー/{member.party.id}] [{client.user.display_name}] {member.display_name} / {member.id} [{member.platform}/{member.input}] がパーティーを離脱\n人数: {member.party.member_count}'))
...
async def event_party_member_kick(member):
    ...
    if data['loglevel'] == 'normal':
        ...
        if member.party.leader.display_name is None:
            if member.display_name is None:
                print(magenta(f'[{now_()}] [パーティー/{member.party.id}] [{client.user.display_name}] None / {member.party.leader.id} [{member.party.leader.platform}/{member.party.leader.input}] が None / {member.id} [{member.platform}/{member.input}] がパーティーからキック\n人数: {member.party.member_count}'))
            else:
                print(magenta(f'[{now_()}] [パーティー/{member.party.id}] [{client.user.display_name}] None / {member.party.leader.id} [{member.party.leader.platform}/{member.party.leader.input}] が {member.display_name} / {member.id} [{member.platform}/{member.input}] がパーティーからキック\n人数: {member.party.member_count}'))
        else:
            if member.display_name is None:
                print(magenta(f'[{now_()}] [パーティー/{member.party.id}] [{client.user.display_name}] {member.party.leader.display_name} / {member.party.leader.id} [{member.party.leader.platform}/{member.party.leader.input}] が None / {member.id} [{member.platform}/{member.input}] がパーティーからキック\n人数: {member.party.member_count}'))
            else:
...
async def event_party_member_promote(old_leader,new_leader):
    ...
    if data['loglevel'] == 'normal':
        ...
        if old_leader.display_name is None:
            if new_leader.display_name is None:
                print(magenta(f'[{now_()}] [パーティー/{new_leader.party.id}] [{client.user.display_name}] None / {old_leader.id} [{old_leader.platform}/{old_leader.input}] から None / {new_leader.id} [{new_leader.platform}/{new_leader.input}] にリーダーが譲渡'))
            else:
                print(magenta(f'[{now_()}] [パーティー/{new_leader.party.id}] [{client.user.display_name}] None / {old_leader.id} [{old_leader.platform}/{old_leader.input}] から {new_leader.display_name} / {new_leader.id} [{new_leader.platform}/{new_leader.input}] にリーダーが譲渡'))
        else:
            if new_leader.display_name is None:
                print(magenta(f'[{now_()}] [パーティー/{new_leader.party.id}] [{client.user.display_name}] {old_leader.display_name} / {old_leader.id} [{old_leader.platform}/{old_leader.input}] から None / {new_leader.id} [{new_leader.platform}/{new_leader.input}] にリーダーが譲渡'))
            else:
                print(magenta(f'[{now_()}] [パーティー/{new_leader.party.id}] [{client.user.display_name}] {old_leader.display_name} / {old_leader.id} [{old_leader.platform}/{old_leader.input}] から {new_leader.display_name} / {new_leader.id} [{new_leader.platform}/{new_leader.input}] にリーダーが譲渡'))
...
async def event_party_member_update(member):
    ...
    if not data['loglevel'] == 'normal':
        if member.display_name is None:
            print(magenta(f'[{now_()}] [パーティー/{member.party.id}] [{client.user.display_name}] None / {member.id} [{member.platform}/{member.input}] パーティーメンバー更新'))
        else:
            print(magenta(f'[{now_()}] [パーティー/{member.party.id}] [{client.user.display_name}] {member.display_name} / {member.id} [{member.platform}/{member.input}] パーティーメンバー更新'))
...
async def event_friend_message(message):
    ...
    if data['loglevel'] == 'normal':
        ...
        if message.author.display_name is None:
            print(f'[{now_()}] [{client.user.display_name}] None / {message.author.id}| {message.content}')
        else:
            print(f'[{now_()}] [{client.user.display_name}] {message.author.display_name}/ {message.author.id} | {message.content}')
...
async def event_party_message(message):
    ...
    if data['loglevel'] == 'normal':
        ...
        if message.author.display_name is None:
            print(f'[{now_()}] [パーティー/{client.user.party.id}] [{client.user.display_name}] None / {message.author.id} | {message.content}')
        else:
            print(f'[{now_()}] [パーティー/{client.user.party.id}] [{client.user.display_name}] {message.author.display_name} / {message.author.id} | {message.content}')
